	...
	*/	
	...
	switch(comando){
		...
		*/
			...
			try{
				var variavelVisita = Browser.visit(site, function (e, browser) {				
					...
					try{
						...
						switch(message.guild.id){
							case '542501711860727848':
								padraoRankWinApex(message, message.member, nickLegivel, level, ["547963888256286732","547959283116146718", "547959283141312525", "547959283904544790"]),[150,100,60,30], "Continua onde está, os niveis atuais são: 150+, 100+, 60+, 30+");;
								mudarNick(message, padraoNickApex(level[0],nickLegivel));
							break;
		...
		case "s":
			if(!(message.author == reifelUser || message.member.roles.has("544981841480777750"))) return;
			switch(parametroUsado){
				case "id":
