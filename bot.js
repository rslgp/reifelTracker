...
client.on('ready', () => {
	...
	client.user.setPresence({
			game: {
				...
				name: "me contrata: www.u.nu/reifelcontato |Reifel#5047| clica assistir >>",
			...
			try{
				...
				request(site, function (error, response, body) {
					...
					level = data.global.level;
						...
						if( (parseInt(levelatual) + 9) >= parseInt(level) ) { 
							switch(message.guild.id){
							case '542501711860727848':
...
function getElo(level, kills, dano){
	...
	var tierS = 55, tierA = 33, tierB = 10, tierAtual = r/10; 
	if(tierAtual >= tierS){
		return ["S",r+""];
